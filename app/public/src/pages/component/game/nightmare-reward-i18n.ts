...
const zhDescriptionMap: Partial<Record<NightmareReward, string>> = {
  [NightmareReward.FINANCIAL_TYCOON]:
    "最大利息 +2；第25回合后提升为 +3。当前 GOLD 小于可获得最大利息所需 GOLD 时，利息翻倍。",
  [NightmareReward.WAR_DIVIDEND]:
    "连胜时额外获得随机 1~2 GOLD（25回合前50%/50%，25回合后10%/90%）。",
  [NightmareReward.SOLO_LEVELING]:
    ...
    }%额外属性；击杀敌方宝可梦时获得${NIGHTMARE_SOLO_LEVELING_KILL_GOLD} GOLD 和 ${NIGHTMARE_SOLO_LEVELING_KILL_EXP} STAR。${NIGHTMARE_SOLO_LEVELING_DURATION_ROUNDS}回合结束后人口恢复。`,
  [NightmareReward.WU_WEI_RULE]:
    "取消利息，改为固定每回合 +2 GOLD 与 +2 STAR；累计击杀150个宝可梦后提升为 +3 GOLD 与 +3 STAR。当达到等级9后，超出的 STAR 将转化为 GOLD。",
  [NightmareReward.LETHAL_TEMPO]:
    "我方所有远程单位累计攻击150次后永久 +8 SPEED。",
  [NightmareReward.CALCULATED_LOSS]:
    "接下来5回合失败时额外获得 +8 GOLD 与 +4 STAR，且玩家损失生命值时回复 2 HP。",
  [NightmareReward.TARGETED_SEARCH]:
    "商店刷新更偏向已触发共鸣的属性池。",
  [NightmareReward.QUALITY_A]:
    "绑定宝可梦基础 ATK/AP 各 -50%，最大 HP -30%（永久且升星不消除）；每击杀1个单位永久 +2 HP、+1 ATK/AP，每回合最多触发10次；每累计触发10次额外永久 +1 DEF 与 +1 SPE_DEF。",
  [NightmareReward.UNYIELDING_DEATH]:
    "我方全体宝可梦每回合首次致死后维持 1 HP 继续5秒。",
  [NightmareReward.BERSERKER]:
    "血量越低，ATK/AP/SPEED 加成越高。",
  [NightmareReward.RESONANCE_EXPERT]:
    "每触发一级共鸣，全队 +10 SPEED（按共鸣级数累积）。",
  ...
  [NightmareReward.SHINRA_TENSEI]:
    "每5秒推开自身半径3格范围内所有敌方单位，且自身 RANGE 永久 +3。",
  [NightmareReward.ASSIST_MASTER]:
    `自身不主动施法，队友施法累计${NIGHTMARE_ASSIST_MASTER_ALLY_CASTS}次后触发自身施法；自身施法时为全队添加自身 MAX_PP 最大值的护盾。`,
  [NightmareReward.DRAGON_DANCE]:
    `战斗开始时全队满 PP，且每${NIGHTMARE_DRAGON_DANCE_INTERVAL_MS / 1000}秒再次触发。`,
  ...
  [NightmareReward.LOYAL_CASTER]:
    `无法普攻，每${NIGHTMARE_LOYAL_CASTER_INTERVAL_MS / 1000}秒自动施法且不消耗 PP。`,
  [NightmareReward.REFRACTION]:
    "减免30%受到的伤害，且将这部分伤害以 TRUE 分摊给自身半径2格内的所有敌方。",
  [NightmareReward.TOXIC_ARMORY]:
    "绑定单位额外获得3个装备栏位；战斗中永久处于3层 POISON（被净化后会重新施加）。",
